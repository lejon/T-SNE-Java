...
public class MatrixOpTest {
	...
	public void testDivides() {
		double [][] A =  {{1,2,3},{3,2,1},{2,1,3}};
		double [][] B = {{4,5,6},{6,5,4},{4,6,5}};
		double [][] Binv = {{1.0/30,11.0/30,-1.0/30},
							{-7.0/15,-2.0/15.0,2.0/3},
							{8.0/15,-2/15,-1.0/3}};
		double [][] ans = {{7.0/10,-3.0/10,0},
				{-3.0/10.0,7.0/10,0},
				{6.0/5.0,1.0/5.0,-1}};
		...
		for (int i = 0; i < Binv.length; i++) {
			for (int j = 0; j < Binv[0].length; j++) {
				System.out.print(Binv[i][j] + ", ");
			}
			System.out.println();
		}

		double [][] BinvC = Matrix.constructWithCopy(B).inverse().getArray();
		

		System.out.println("Binv:");
		for (int i = 0; i < Binv.length; i++) {
			for (int j = 0; j < Binv[0].length; j++) {
				System.out.print(Binv[i][j] + "=>" + BinvC[i][j] + ", ");
			}
			System.out.println();
		}
		
		double [][] AxBinvC = Matrix.constructWithCopy(A)
				.times(Matrix.constructWithCopy(B).inverse()).getArray();

		System.out.println("AxBinvC:");
		for (int i = 0; i < Binv.length; i++) {
			for (int j = 0; j < Binv[0].length; j++) {
				System.out.print(AxBinvC[i][j] + ", ");
			}
			System.out.println();
		}
		...
		double [][] tr1 = TSne.divide(A,B);
		double [][] back = multiply(tr1,A);
		System.out.println("Matrices:");
		...
		for (int i = 0; i < tr1.length; i++) {
			for (int j = 0; j < tr1[0].length; j++) {
				assertEquals(tr1[i][j],ans[i][j],0.0000001);
			}
		}
	}
